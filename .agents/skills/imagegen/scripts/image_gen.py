#!/usr/bin/env python3
"""CLI for explicit image generation or editing with GPT Image models.

Used by the opencode imagegen skill for OpenAI-compatible Image API workflows.
...
def _ensure_api_key(args: argparse.Namespace) -> None:
    if _resolve_api_key(args):
        print("Image API key is set.", file=sys.stderr)
        return
    if args.dry_run:
        _warn("Image API key is not set; dry-run only.")
        return
    _die("Image API key is not set. Set IMAGE_GEN_API_KEY or OPENAI_API_KEY, or pass --api-key.")
...
def _add_shared_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--api-key", help="Image API key. Prefer IMAGE_GEN_API_KEY or OPENAI_API_KEY to avoid shell history leaks.")
    parser.add_argument("--base-url", help="OpenAI-compatible API base URL. Defaults to IMAGE_GEN_BASE_URL, OPENAI_BASE_URL, or the OpenAI SDK default.")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="CLI for explicit image generation or editing via GPT Image models"
    )
