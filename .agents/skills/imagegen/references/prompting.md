...
These prompting principles apply to the `scripts/image_gen.py` CLI workflow used by this opencode skill.
...
## Transparent images
- Use CLI `gpt-image-2` with chroma-key removal first for transparent-image requests. If the subject is clearly too complex for chroma-key removal, explain the `gpt-image-1.5` true-transparency fallback and ask before switching models.
...
- After generation, remove the background locally with `python "<skill-dir>/scripts/remove_chroma_key.py" --input <source> --out <final.png> --auto-key border --soft-matte --transparent-threshold 12 --opaque-threshold 220 --despill`, then validate the alpha result before shipping it.
...
- If a transparent request needs true transparency, ask before using `gpt-image-1.5` unless the user already explicitly chose it. Explain that chroma-key removal is the default path, but `gpt-image-2` does not support `background=transparent`.
