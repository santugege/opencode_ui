...
This file documents the API parameters used by the `scripts/image_gen.py` CLI workflow.
...
`gpt-image-2` does not currently support the Image API `background=transparent` parameter. The skill's default transparent-image path is CLI `gpt-image-2` with a flat chroma-key background, followed by local alpha extraction with `python "<skill-dir>/scripts/remove_chroma_key.py"`.
...
## Important boundary
- `quality`, `input_fidelity`, explicit masks, `background`, `output_format`, and related parameters are CLI execution controls, not prompt text.
