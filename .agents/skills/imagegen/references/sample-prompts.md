...
These prompt recipes are for the `scripts/image_gen.py` CLI workflow used by this opencode skill.
...
Execution details such as explicit CLI flags, `quality`, `input_fidelity`, masks, output formats, and local output paths belong in the CLI command. For transparent images, prompt for a flat chroma-key background and remove it locally with `python "<skill-dir>/scripts/remove_chroma_key.py"`; use `gpt-image-1.5 --background transparent --output-format png` only when the user explicitly confirms true model-native transparency or already requested `gpt-image-1.5`.
